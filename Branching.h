...
class Branching : public Scene
{

public:
	Branching(const char* name) : Scene(name) {}
	...
	void growCell()
	{
		...
		std::cout << "Number " << numSimulationParticles << '\n';
		int endParticleIndex = numSimulationParticles - 1;


		//Here we want to find a postion around the initial particles then expand to any particle
		for (int i = 0; i < 50; ++i) //change i<x if need more positions. (x,y,z). 
		{
